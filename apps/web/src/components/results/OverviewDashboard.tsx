...
const formatPayback = (months: number | null | undefined, status?: string) => {
  if (status === 'NOT_PROFITABLE' || status === 'NOT_REACHED') return 'Not reached'
  if (months == null) return 'N/A'
  return status === 'BEYOND_HORIZON' ? `${months} months (beyond horizon)` : `${months} months`
}
  ...
  return (
    <div className="space-y-6">
      ...
      {baseROI && (
        ...
        <MetricCard
          label="Break-even"
          value={formatPayback(baseROI.breakeven_months, baseROI.payback_status)}
          accent="blue"
        />
      </div>
    )}
