...
class EconomistAgent(BaseAgent):
    ...
    def run(self, state: dict[str, Any], request: PlanRequest) -> dict[str, Any]:
        ...
        total_opex = sum(cb.opex_per_acre * land * pe.area_fraction for cb, pe in zip(cost_breakdowns, portfolio))
        ...
        for scenario, yield_key, price_key, loss_key in [
            ...
            breakeven = None
            if profit > 0 and horizon_months > 0:
                monthly_profit = profit / max(horizon_months, 1)
                if monthly_profit > 0:
                    breakeven = round(total_cost / monthly_profit, 1)
